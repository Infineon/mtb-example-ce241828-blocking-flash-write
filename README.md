...
This code example shows how to write a flash memory row.

[View this README on GitHub.](https://github.com/Infineon/mtb-example-ce241828-blocking-flash-write)

[Provide feedback on this code example.](https://cypress.co1.qualtrics.com/jfe/form/SV_1NTns53sK2yiljn?Q_EED=)
...
The following example clones the "[mtb-example-ce241828-blocking-flash-write](https://github.com/Infineon/mtb-example-ce241828-blocking-flash-write)" application with the desired name "BlockingFlashWrite" configured for the *KIT_PSOC4-HVMS-128K_LITE* BSP into the specified working directory, *C:/mtb_projects*:

   ```
   project-creator-cli --board-id KIT_PSOC4-HVMS-128K_LITE --app-id mtb-example-ce241828-blocking-flash-write --user-app-name BlockingFlashWrite --target-dir "C:/mtb_projects"
   ```
...
4. Set the value of the `MAKE_FLASH_WRITE_FAIL` macro in the *main.c* file to '1', rebuild the project, reprogram the device, and observe that the LED_ERROR turns ON to indicate that the flash write operation is failed

   LED signal name | Test results | KIT_PSOC4-HVMS-128K_LITE, KIT_PSOC4-HVMS-64K_LITE | KIT_PSOC4-HVPA-144K_LITE |  KIT_PSOC-HVPA-SPM1_LITE
   ---------|-------------|-----------|-----------|-----------------
	LED_OK | Flash write successful | LED7 (P0.6) | LED6 (P0.3) | LED6 (P0.5)
	LED_ERROR | Flash write operation failed | LED6 (P0.7) | LED7 (P0.2) | LED7 (P2.6)
...
1. Defines the size of the flash memory row and initializes data to write
2. Initializes and board peripherals
3. Sets the write data
4. Configures LEDs for results (LED_OK / LED_ERROR)
5. Writes data to flash memory row and confirms read data
6. Confirms results on the LED_OK or LED_ERROR. If the flash write is successful, LED_OK turns ON. Otherwise, the LED_ERROR turns ON
...
**Table 1. Application resources**
 ...
 :------- | :------------    | :------------ 
 GPIO | LED_OK | Shows success and failure information
 GPIO | LED_ERROR | Shows success and failure information
...
Document title: *CE241828* – *PSOC&trade; 4: Blocking flash write*
